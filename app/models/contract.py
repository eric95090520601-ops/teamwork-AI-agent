...
ILLEGAL_CLAUSES = [
    ...
    # ===== 押金類 =====
    {
        ...
        "keywords": [r"押金.{0,5}(三|四|五|六|3|4|5|6).{0,2}個?(月|月份)", r"押金超過", r"保證金.{0,5}(三|四|五|六|3|4|5|6).{0,2}個?(月|月份)", r"押金.{0,10}不得抵扣", r"押金.{0,10}不能抵扣"],
    ...
    },
    {
        ...
        "keywords": [r"押金不退", r"押金概不退還", r"押金恕不退還", r"押金不予退還", r"保證金不退", r"沒收押金"],
    ...
    # ===== 修繕責任類 =====
    {
        ...
        "keywords": [r"一切修繕", r"所有修繕", r"全部維修", r"修繕費用.{0,5}(由承租人|自行|由租客|由乙方)負擔", r"維修費用由租客", r"修繕費用由乙方", r"耗材.{0,5}乙方負擔", r"修繕.{0,5}不負責"],
        "law": "民法 第429條",
    ...
    },
    {
        ...
        "keywords": [r"漏水自行處理", r"設備故障自行維修", r"冷氣故障不負責", r"熱水器故障自理", r"房東不負責修繕", r"出租人不負修繕", r"自然損壞.{0,5}乙方維修", r"自然損壞.{0,5}不負責"],
        "law": "民法 第429條、第435條",
    ...
    # ===== 居住權保障類 =====
    {
        ...
        "keywords": [r"不得設籍", r"禁止設籍", r"不得辦理戶籍", r"禁止辦理戶籍登記", r"不得遷入戶籍", r"戶籍不得遷入", r"禁止遷入", r"不能入籍"],
        "law": "戶籍法 第16條",
    ...
    },
    {
        ...
        "keywords": [r"不得申請補助", r"禁止申請補貼", r"不得申請租屋補貼", r"禁止申請政府補助", r"不可申請租金補助", r"放棄申請.*補貼", r"申請補助.{0,5}違約"],
        "law": "住宅法 第54條",
    ...
    },
    {
        ...
        "keywords": [r"不得報稅", r"禁止報稅", r"不得申報租金", r"禁止申報扣除", r"不可報稅", r"不得扣除租金", r"報稅.{0,5}補貼稅金", r"申報.*加收", r"稅金.*乙方負擔"],
        "law": "所得稅法 第17條",
    ...
    # ===== 合約終止類 =====
    {
        ...
        "keywords": [r"房東得隨時終止", r"出租人得隨時解約", r"隨時要求搬遷", r"房東可隨時收回", r"隨時終止本契約", r"立即搬離", r"無條件搬離", r"甲方得隨時收回", r"甲方得終止"],
        "law": "民法 第450條、第456條",
    ...
    },
    {
        ...
        "keywords": [r"承租人不得提前終止", r"乙方不得解約", r"不得提前退租", r"禁止提前解約", r"租客不得中途退租", r"提前解約.{0,5}沒收押金", r"不得終止租約"],
    ...
    # ===== 違約金條款 =====
    {
        ...
        "keywords": [r"違約金.{0,5}(三|四|五|六|3|4|5|6).{0,2}個?月", r"賠償.{0,5}(三|四|五|六|3|4|5|6).{0,2}個?月", r"賠償半年租金", r"罰款.{0,5}(三|四|五|六|3|4|5|6).{0,2}個?月", r"沒收全部押金"],
        "law": "民法 第252條",
    ...
    # ===== 隱私與進入權類 =====
    {
        ...
        "keywords": [r"房東得隨時進入", r"出租人得隨時進入", r"房東可隨時進屋", r"不需通知進入", r"無需預約進入", r"隨時查看", r"甲方有權.{0,5}進入", r"甲方得.*進入"],
    ...
    # ===== 費用相關類 =====
    {
        ...
        "keywords": [r"電費每度\s*[6-9]\s*元", r"電費每度\s*[1-9][0-9]\s*元", r"自訂電費", r"電費另計高於", r"每度電.{0,5}[6-9]元", r"夏季電費.{0,5}[8-9]元"],
        "law": "消費者保護法 第12條、電業法",
    ...
    },
    {
        ...
        "keywords": [r"另收管理費", r"額外服務費", r"清潔費另計", r"行政費另收", r"手續費另計", r"雜費另計", r"走廊燈費", r"公用電費另計"],
        "law": "消費者保護法 第12條",
    ...
    # ===== 定型化契約類 =====
    {
        ...
        "keywords": [r"當場簽約", r"立即簽署", r"現場簽約有效", r"不得要求審閱", r"簽約後不得反悔", r"放棄審閱期", r"已充分審閱", r"不需審閱"],
        "law": "消費者保護法 第11條之1",
    ...
    },
    {
        ...
        "keywords": [r"出租人得修改", r"房東得變更", r"條款得隨時調整", r"房東保留修改權", r"得單方修改", r"得片面變更", r"甲方保留.*權利", r"甲方得視情況變更"],
        "law": "消費者保護法 第12條",
...
def analyze_text(contract_text: str) -> list:
    """文字分析模式：正則表達式比對，找出潛在違法條款"""
    import re
    found_issues = []
    # 移除空白符號以提高配對精準度
    text_clean = re.sub(r'\s+', '', contract_text)

    for clause in ILLEGAL_CLAUSES:
        for keyword in clause["keywords"]:
            if re.search(keyword, text_clean):
                found_issues.append({
                    "id": clause["id"],
